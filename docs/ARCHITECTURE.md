...
长字幕不会直接进入单次全量预填充。网关先截取 `min(5 分钟, 视频总时长 / 3)` 的开场时间窗，流出一个一级标题和首章；随后把完整字幕与已经展示的开场作为第二次请求上下文，只续写新的二级章节。两个 Gemini SSE 流被服务端串接为同一条 NDJSON 响应，浏览器不需要理解阶段边界。若开场时间窗已经覆盖全部字幕，则只发起一次请求。

这里选择两阶段而非固定五分钟分片：目标是降低首字延迟，不是把文章变成相互独立的小摘要。第二阶段仍能看到完整视频，因此可以规划中段和结尾主题；最多两次请求也更适合 Gemini 免费额度，并降低任一分片失败导致整篇文章不可用的概率。

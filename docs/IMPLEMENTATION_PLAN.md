...
- [x] 写 URL、章节和任意分块 NDJSON 的失败测试。
- [x] 运行 `pnpm vitest run tests/unit/shared`，确认因模块缺失失败。
- [x] 写最小纯函数实现并补齐共享 TypeScript 联合类型。
- [x] 再次运行测试，确认共享协议测试通过。
- [x] 提交 `feat: establish shared generation contracts`。
...
- [x] 写跨网络分块 SSE、提示词注入边界和六字段校验失败测试。
- [x] 运行目标测试并确认缺失行为导致失败。
- [x] 实现 Gemini REST 调用、SSE 解析和结构化输出校验。
- [x] 运行 Gemini 单元测试并检查错误映射。
- [x] 提交 `feat: add gemini streaming and structured summaries`。
...
- [x] 写集成测试，证明 delta 先于 completed，状态被保存，5W1H 请求不含文章。
- [x] 运行 `pnpm test:integration`，确认路由和对象缺失导致失败。
- [x] 实现 Hono 入口、Durable Object 状态机、Alarm 清理和 NDJSON 输出。
- [x] 运行集成测试并确认流式顺序、失败事件和总结上下文通过。
- [x] 提交 `feat: persist generation sessions at the edge`。
...
- [x] 写表单校验、增量显示、Fixture 标记和六字段渲染失败测试。
- [x] 运行客户端测试，确认组件缺失导致失败。
- [x] 实现双栏工作台、响应式单栏、系统深色模式和完整交互状态。
- [x] 运行组件测试并检查键盘语义与错误反馈。
- [x] 提交 `feat: build the streaming article workspace`。
...
- [x] 写 Fixture 端到端测试，覆盖提交、流式正文、来源标记和 5W1H 六字段。
- [x] 运行端到端测试，确认完整用户路径。
- [x] 执行 `pnpm lint && pnpm typecheck && pnpm test && pnpm test:integration && pnpm test:e2e && pnpm build`。
- [x] 在 390px、768px 和 1440px 检查页面截图及系统深色模式。
- [x] 更新 README，记录字幕、Gemini 流、用户约束、5W1H、取舍、测试和部署。
- [x] 提交 `docs: document architecture and delivery evidence`。
- [x] 创建公开 GitHub 仓库并推送当前分支。
- [x] 配置 Cloudflare Secret，执行 `pnpm deploy`，用公开 URL 完成 smoke test。

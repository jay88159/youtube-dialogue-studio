...
改造前对参考视频做生产分段计时：`transcript.ready` 在 1.51 秒到达，第一个 `article.delta` 在 9.50 秒到达。字幕准备后仍等待 7.99 秒，证明主要延迟来自 Gemini 对约 10 万字符字幕的全量 Prompt 预填充，而不是字幕提取。

版本 `efb4aa7f-403f-4e16-8c94-66e6d8287c21` 将文章改为两阶段：先读取 `min(5 分钟, 总时长 / 3)` 的开场字幕并流出首章，再携带完整字幕与已展示内容续写全文。同一参考视频再次生产验证：

| 指标 | 改造前 | 两阶段 |
| --- | ---: | ---: |
| 字幕就绪 | 1.51 s | 1.50 s |
| 首个正文增量 | 9.50 s | 7.61 s |
| 字幕就绪到首字 | 7.99 s | 6.12 s |

首字等待下降约 24%。Gemini 请求仍有网络与模型调度的固定开销，因此不能承诺即时响应。最终文章包含 1 个一级标题、10 个二级章节、10837 个字符和 264 个真实增量，没有重复标题；章节 5W1H 继续从服务端完整上下文生成，实测 `When` 覆盖 20 世纪 30 年代、1943 年、当前 AI 爆发早期和未来 5–10 年。

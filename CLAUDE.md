...
12. 原始字幕主路径使用 `youtube-caption-extractor`；不要同时维护另一套 watch-page 解析器。
13. 参考 Fixture 必须由字幕轨道可重复生成并覆盖完整视频，不使用人工摘要代替原始字幕。
14. 主文章遵循 `docs/ARTICLE_OUTPUT_CONTRACT.md`；长视频不得退化为只覆盖少数主题的短摘要。
15. 长字幕采用两阶段渐进生成：首批时间窗不超过 5 分钟且不超过总时长三分之一，续写阶段必须读取完整字幕并且不得重复一级标题或开场章节。
